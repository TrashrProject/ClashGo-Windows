...
if (-not (Test-Path $opencvHeader)) {
    if (-not $InstallOpenCV) {
        ...
        try {
            ...
            (Get-Content .\win_build_opencv.cmd -Raw).Replace("-DBUILD_PERF_TESTS=ON", "-DBUILD_PERF_TESTS=OFF -DBUILD_LIST=core,face,videoio,imgproc,highgui,imgcodecs,objdetect,features2d,video,dnn,xfeatures2d,plot,tracking,img_hash,calib3d,bgsegm,photo,aruco,wechat_qrcode,ximgproc,mcc") |
