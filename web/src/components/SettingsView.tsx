...
const SettingsView: React.FC<SettingsViewProps> = React.memo(({
  ...
  appVersion, updateStatus, onCheckUpdates, onClearSkip, systemDiagnostics,
}) => {
        ...
        <div className="bg-zinc-950 dark:bg-black text-white p-6 rounded-2xl border border-zinc-800 shadow-xl">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              { label: 'ADB access', ok: systemDiagnostics?.emulator.adb_enabled ?? false, value: systemDiagnostics?.emulator.adb_enabled ? 'Enabled' : (systemDiagnostics?.emulator.adb_setting_present ? 'Disabled (auto-fix on Start)' : 'Check in BlueStacks') },
            ].map((item) => (
